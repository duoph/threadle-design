...
const BrowseDressByCategory = () => {
    ...
    return (
        <div className='lg:px-10 px-5'>
            <div className='h-full rounded-md bg-td-secondary gap-5 flex flex-col pb-10 lg:px-20 px-5'>
                ...
                <div className='flex flex-wrap items-center justify-center w-full gap-2 font-medium text-sm'>
                    {categories?.slice(0, 6).map((category: Category, i) => (
                        <div key={i} onClick={() => router.push(`/category/${category._id}`)} className='bg-emerald-50 md:w-1/3 w-full  rounded-md py-2 flex items-center justify-center overflow-hidden cursor-pointer'>
                            <h1 className=' text-[20px] font-normal text-center'>{category.categoryName}</h1>
                        </div>
                    ))}
                </div>
                <div className='flex items-center justify-center'>
                    <div className=' cursor-pointer flex items-center justify-center text-white border border-white md:w-1/4 w-full rounded-md  py-3'>
